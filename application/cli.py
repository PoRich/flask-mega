...
''' current_app does not work in this case because these CLI commands are
registered at start up, not during the handling of a request, which is the
only time when current_app can be used'''
def register(app):
    @app.cli.group()
    def translate():
